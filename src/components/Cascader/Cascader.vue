...
const cpMaxLevel = computed(() => {
  return typeof props.loadData === "function"
    ? props.maxLevel
...
async function onLoadData(selectedOptions) {
  ...
  await getData(
    targetOption[props.fieldNames.value] ?? 0,
...
function onChange(value, selectedOptions) {
  ...
  emit(
    ...
    selectedOptions.map((item) =>
      pick(item, [props.fieldNames.label, props.fieldNames.value]),
    ),
  );
  onFieldChange();
}
