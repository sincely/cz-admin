<template>
  <div ref="infiniteScrollRef" class="x-infinite-scroll" @scroll="check">
    <div class="x-infinite-scroll__body">
      <slot></slot>
    </div>
    <div class="x-infinite-scroll__footer">
      <div v-if="loading" class="x-infinite-scroll__text">
        <slot name="loading">
          {{ loadingText }}
        </slot>
      </div>
      <div v-if="finished" class="x-infinite-scroll__text">
        <slot name="finished">
          {{ finishedText }}
        </slot>
      </div>
      <div v-if="error" class="x-infinite-scroll__text x-infinite-scroll__text--error" @click="handleError">
        <slot name="error">
          {{ errorText }}
        </slot>
      </div>
