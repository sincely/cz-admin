<template>
  <a-page-header main title="基础详情页" :ghost="false"></a-page-header>
  <a-card>
    ...
    <a-divider></a-divider>
    <h3>退货商品</h3>
    <a-table row-key="id" :columns="goodsColumns" :data-source="goodsList" :pagination="false">
      <template #summary>
        <a-table-summary>
          <a-table-summary-row class="fw-600">
            <a-table-summary-cell :index="0" :col-span="4">总计</a-table-summary-cell>
            <a-table-summary-cell class="align-right" :index="1">10</a-table-summary-cell>
            <a-table-summary-cell class="align-right" :index="2">61.5</a-table-summary-cell>
          </a-table-summary-row>
        </a-table-summary>
      </template>
    </a-table>
    <h3 class="mt-8-3">退货进度</h3>
    <a-table row-key="id" :columns="progressColumns" :data-source="progressList" :pagination="false">
      <template #bodyCell="{ text, column }">
        <template v-if="column.key === 'key3'">
          <a-badge v-if="text === 1" status="processing" text="进行中" />
          <a-badge v-if="text === 2" status="success" text="成功" />
        </template>
      </template>
    </a-table>
  </a-card>
