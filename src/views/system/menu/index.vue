    ...
    >
      <template #bodyCell="{ column, record }">
        <template v-if="'menuType' === column.key">
          <!--菜单-->
          <a-tag v-if="menuTypeEnum.is('menu', record.menu_type)" color="processing">
            {{ menuTypeEnum.getDesc(record.menu_type) }}
          </a-tag>
          <!--按钮-->
          <a-tag v-if="menuTypeEnum.is('button', record.menu_type)" color="success">
            {{ menuTypeEnum.getDesc(record.menu_type) }}
          </a-tag>
        </template>
        <template v-if="'action' === column.key">
          ...
          <x-action-button tag="span">
            <a-dropdown :trigger="['click']">
              ...
              <template #overlay>
                <a-menu>
                  <a-menu-item @click="$refs.editDialogRef.handleEdit(record)">新建下级</a-menu-item>
