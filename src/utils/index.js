  ...
  data.forEach((item, index, array) => {
    ...
    for (let filedKey in fieldNames) {
      ...
      if (filedValue === treeFieldName) {
        ...
        temp[filedKey] =
          typeof record !== "undefined" && record !== "" ? record : "";
      }
    }
...
export const findTree = (
  ...
  value,
  callback,
  fieldNames = { key: "id", children: "children" },
  path = [],
) => {
  data.forEach((item, index, array) => {
    if (
      isObject(value) ? isMatch(item, value) : item[fieldNames.key] === value
    ) {
      return callback(item, index, array, [...path, item]);
    }
    if (item[fieldNames.children]) {
      return findTree(item[fieldNames.children], value, callback, fieldNames, [
        ...path,
        item,
      ]);
    }
