      ...
      <a-tab-pane v-for="(item, index) in cpMultiTabList" :key="index">
        <template #tab>
          <a-dropdown :trigger="['contextmenu']">
            <div>
              {{ item.meta.title }}
              <span v-if="cpCurrent === index" class="multi-tab__icon" @click.stop="handleReload(item)">
                <reload-outlined :spin="spin" class="ma-0" />
              </span>
              <template v-if="cpShowCloseBtn">
                <span class="multi-tab__icon" @click.stop="handleClose(item)">
            ...
            <template #overlay>
              <a-menu>
                <a-menu-item key="reload" @click="handleReload(item)">重新加载</a-menu-item>
                <template v-if="cpShowCloseBtn">
                  <a-menu-item key="close" @click="handleClose(item)">关闭</a-menu-item>
                </template>
                <template v-if="cpShowCloseOtherBtn">
                  <a-menu-item key="closeOther" @click="handleCloseOther(item)">关闭其他</a-menu-item>
                </template>
                <a-menu-item v-if="index > 0" key="closeLeft" @click="handleCloseLeft(item)">关闭左侧</a-menu-item>
                <a-menu-item v-if="index < cpMultiTabList.length - 1" key="closeRight" @click="handleCloseRight(item)">
                  关闭右侧
                </a-menu-item>
              </a-menu>
            </template>
